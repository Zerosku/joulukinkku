...
public class DialogueCandyCane : MonoBehaviour
{
    // this script is for the dialogue of pig who wants a candy cane
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        ...
        {
            touch = true;
            dialog.text = "Find me some of that CandyCane drug??";
        }
    }
    ...
    void Update()
    {
        ...
        if (touch == true)
        {
            ...
            {
                if (inv.CandyCane.activeSelf)
                {
                    dialog.text = "Awww yeeeeaaahhhh.";
                    Destroy(this);
                    gm.points++;
                    gm.karma--;
                    Debug.Log(gm.karma);
                    inv.CandyCane.SetActive(false);
                }
                else if (!inv.CandyCane.activeSelf)
                {
                    dialog.text = "Hurry up I'm gettin paranoid!";
                }
            }
            else
            {
